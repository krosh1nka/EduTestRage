<template>
	<div class="SummerLottery">
		<div :class="`SummerLottery__lottery-card SummerLottery__lottery-card-${LotteryColor}`">
			<div class="SummerLottery__lottery-card-main">
				...
				<div class="SummerLottery__lottery-card-award-section" v-if="awardList">
					<div class="SummerLottery__lottery-card-award-item" :key="award.Name" v-for="award in awardList">
						<div class="SummerLottery__lottery-card-award-item-stars">
							<div :class="`SummerLottery__lottery-card-award-item-star SummerLottery__lottery-card-award-item-star-${award.Probability}`"
								:key="i" v-for="i in award.Probability"></div>
						</div>
		...
		<div class="SummerLottery__right-info-section">
			<div class="SummerLottery__info-item">
				<img src="./Assets/KeyIcons/SummerLottery_key-left.png" />
				<div class="SummerLottery__info-item-text">
					<div>Предыдущий билет</div>
					<div>Кнопка влево</div>
				</div>
			</div>
			<div class="SummerLottery__info-item">
				<img src="./Assets/KeyIcons/SummerLottery_key-right.png" />
				<div class="SummerLottery__info-item-text">
					<div>Следующий билет</div>
					<div>Кнопка влево</div>
				</div>
			</div>
			<div class="SummerLottery__info-item">
				<img src="./Assets/KeyIcons/SummerLottery_key-space.png" />
				<div class="SummerLottery__info-item-text">
					<div>Следующий билет</div>
					<div>Кнопка влево</div>
				</div>
		...
		<div class="SummerLottery__info">
			Нажмите чтобы стереть за 10 000
		</div>
	</div>
</template>
<script>
export default {
  ...
  methods: {
    ...
    bindKeyDown (e) {
      ...
      if (hendlers[e.keyCode]) hendlers[e.keyCode]()
    },
    setSendCanvas () {
      ...
      const updateParticles = () => {
        ...
        for (let i = 0; i < this.particles.length; i++) {
          ...
          ctx.fill()
          // Уменьшение радиуса частицы со временем
          particle.radius -= 0.04
          // Удаление частицы, если ее радиус становится слишком малым
  ...
  mounted () {
    ...
    this.setSendCanvas()
    const data = { Prizes: [{ Name: '50 EXP', Probability: 1, Image: 'EXP' }, { Name: '1500$', Probability: 1, Image: 'Money' }, { Name: '25 EXP', Probability: 1, Image: 'EXP' }, { Name: '1000$', Probability: 1, Image: 'Money' }], IndexTakePrize: 3, Color: 'Red' }
    ...
    if (typeof mp !== 'undefined') {
      mp.events.add('SummerLottery:Load:Cef', this.load)
  ...
  unmounted () {
    ...
    if (typeof mp !== 'undefined') {
      mp.events.remove('SummerLottery:Load:Cef', this.load)
    }
  }
}
</script>
