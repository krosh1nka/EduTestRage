<template>
	<div class="GasStantion" v-if="!active">
		<div class="GasStantion_content">
			<!-- <img class="GasStantion_header-hr" src="./Assets/GasStantion_hr.png" /> -->
			<div>
				<div class="GasStantion_header">
					...
					<div class="PlayerMenu__sidebar-playerInfo">
						<div class="GasStantion_header-money-item">
							...
							<div>
								<div>Наличные</div>
								<div>{{ $formatCurrency(gasData.playerData.money) }} $</div>
							</div>
						</div>
						<div class="GasStantion_header-money-item">
							...
							<div>
								<div>Карта</div>
								<div>{{ $formatCurrency(gasData.playerData.card) }} $</div>
							</div>
						</div>
						<div>
				...
				<div class="GasStantion-main">
					<div class="GasStantion-main-info">
						<div class="GasStantion-main-info-item">
							...
							<!-- <div class="GasStantion-main-info-item-name">{{ $GetParamsVehicleByModelName(gasData.vehModel).realName }}</div> -->
							<div class="GasStantion-main-info-item-info">Для вашего автомобиля доступен вид топлива
							<span>“{{ gasData.vehTypeFuel }}”</span></div>
							<div class="GasStantion-main-info-item-gas-info">
								<div>
									<div>Топливо в баке:</div> <span>{{ gasData.fuelMin }} L</span>
								</div>
								<div class="GasStantion-main-info-item-gas-info-range">
									<div class="GasStantion-main-info-item-gas-info-range-item"
										:class="{ 'GasStantion-main-info-item-gas-info-range-item--full': i <= Math.floor(getProcentCurrentData / 10) }" :key="i"
										v-for="i in 9"></div>
								</div>
							</div>
						</div>
						<div class="GasStantion-main-select-section">
							...
							<div class="GasStantion-main-select-info">
								<div>За литр <span>{{ $formatCurrency(gasData.priceForLiter) }} $</span></div>
								<div>Будет залито <span>{{ Math.abs(gasData.fuelCurrent - gasData.fuelMin) }} L. </span></div>
							</div>
							<div class="GasStantion-main-select-input-section">
								<div :style="{ left: (getProcent(gasData) > 97 ? 97 : getProcent(gasData)) + '%' }">{{ gasData.fuelCurrent + "л." }}</div>
								<input type="range" @change="(e) => this.selectBuyItem({ Type: 'Fuel', Value: Number(gasData.fuelCurrent) })"
									:style="{ background: `linear-gradient(90deg, #FFF ${getProcent(gasData) + '%'}, rgba(115, 115, 147, 0.10) 0%)` }"
									:min="gasData.fuelMin === gasData.maxFuel ? gasData.fuelMin -1 : gasData.fuelMin" :max="gasData.maxFuel" v-model="gasData.fuelCurrent" />
							</div>
						</div>
					</div>
					<div class="GasStantion-main-product-list">
						<div class="GasStantion-main-product-list-item" :key="key" v-for="(item, key) in getItemsInfo">
							...
							<div class="GasStantion-main-product-list-item-buy-section">
								<div>{{ item.Name }}</div>
								<button @click="selectBuyItem({ Type: 'Item', Value: key })">
									<div>$ {{ $formatCurrency(getPriceItem(key)) }}</div>
								</button>
							</div>
						</div>
					</div>
				</div>
				<div class="GasStantion-buy-section" v-if="selectItem != null">
					<img src="./Assets/GasStantion-cart-bg.svg" />
					<div class="GasStantion-buy-section-content">
						<div class="GasStantion-buy-section-header">
							<div>Gas Stantion #{{ gasData.bizInfo.id }}</div>
							<div>{{ gasData.serverTime }}</div>
						</div>
						<div class="GasStantion-buy-section-title">Cтоимость</div>
						<div class="GasStantion-buy-section-amount">$ {{ $formatCurrency(getPrice) }}</div>
						<div class="GasStantion-buy-section-buttons">
							<button @click="buyItem('cash')">Наличными</button>
							<button @click="buyItem('card')">Картой</button>
						</div>
					</div>
				</div>
			</div>
...
const ItemsInfo = {
  canister: {
    ...
    Info: 'че-то написать надо',
    Image: 'GasStantion_Canister.png'
  },
  repairKit: {
    Name: 'Рем. комплект',
    Info: 'че-то написать надо',
    Image: 'GasStantion_RemKomplekt.png'
  },
  canisterThreeKit: {
    Name: 'Набор рем. комплектов (3 шт.)',
    Info: 'че-то написать надо',
    Image: 'GasStantion_ThreeCanister.png'
  }
}
export default {
  data () {
  ...
  computed: {
    getPrice () {
      ...
      switch (this.selectItem.Type) {
        case 'Fuel':
          const fuelCount = Math.abs(this.gasData.fuelCurrent - this.gasData.fuelMin)
          return Math.abs(fuelCount * this.gasData.priceForLiter)
        case 'Item':
          return this.getPriceItem(this.selectItem.Value)
      }
      return 0
    },
    getItemsInfo () {
    ...
    getProcentCurrentData () {
      return this.gasData.fuelMin / (this.gasData.maxFuel) * 100
    }
  },
  methods: {
    getPriceItem (itemType) {
      const price = this.gasData.items[itemType]
      if (this.gasData.bizInfo.nacenka <= 0) {
        return price
      }
      return parseInt(price + (price / 100 * this.gasData.bizInfo.nacenka))
    },
    selectBuyItem (item) {
    ...
    getProcent (data) {
      ...
      if (data.maxFuel == data.fuelCurrent) {
        return 100
      }
      return (data.fuelCurrent - data.fuelMin) / (data.maxFuel - data.fuelMin) * 100
    },
    buyItem (typeCash) {
      mp.trigger('GasStantion:Buy:Client', String(typeCash), String(this.selectItem.Type), this.selectItem.Value)
    },
    exitMenu () {
      mp.trigger('GasStantion:Close:Client')
    },
    open (json) {
      this.gasData = JSON.parse(json)
  ...
  mounted () {
    if (typeof mp !== 'undefined') {
      mp.events.add('GasStantion:Open:Cef', this.open)
      mp.events.add('GasStantion:UpdateData:Cef', this.updateData)
      mp.trigger('GasStantion:AwaitMounted')
    }
  },
  unmounted () {
    if (typeof mp !== 'undefined') {
      mp.events.remove('GasStantion:Open:Cef', this.open)
      mp.events.remove('GasStantion:UpdateData:Cef', this.updateData)
    }
    this.active = false
    this.selectItem = null
  }
